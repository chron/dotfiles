...
alias git-branches="git for-each-ref --color --sort=committerdate refs/heads/ --format='%(HEAD) %(color:yellow)%(refname:short)%(color:reset) - %(color:red)%(objectname:short)%(color:reset) - %(contents:subject) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))' |tail -n 15"
alias git-branch-files="git log master..head --name-only | grep '^[a-z]' | grep -v '^commit' | sort | uniq"
alias lnb="ruby ~/scripts/last-n-branches.rb"
